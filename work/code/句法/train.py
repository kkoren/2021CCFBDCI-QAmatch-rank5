...
def evaluate(model, criterion, metric, data_loader):
    ...
    for batch in data_loader:
        input_ids,token_type_ids,adjacency_matrix,head_ids,labels = batch
...
def do_train_1fold():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            #model返回预测值和损失
            logits1 = model(input_ids=input_ids,token_type_ids=token_type_ids,adjacency_matrix=adjacency_matrix,head_ids=head_ids)
            ...
            if global_step % args.eval_step == 0 and rank == 0:#每一百步验证
                ...
                if accuracy > best_accuracy:
                    save_dir = os.path.join('./model_data/'+args.model_name+'_ddparser2/'+args.save_checkpoints_name, "model_%d_%.5f" % (global_step,accuracy))
