...
def do_train_1fold():
    ...
    #构建模型
    model = QuestionMatching_BN(pretrained_model, rdrop_coef=args.rdrop_coef)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            ce_loss = criterion(logits1, labels)#交叉熵
            ...
            if global_step % args.eval_step == 0 and rank == 0:#每一百步验证
                ...
                if accuracy > best_accuracy:
                    save_dir = os.path.join('./model_data/'+args.model_name+'_gelu/'+args.save_checkpoints_name, "model_%d_%.5f" % (global_step,accuracy))
