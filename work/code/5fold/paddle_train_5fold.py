...
def stratifiedk_5fold():
    # 加载训练集
    bq_df = pd.read_csv('bq_df.csv',names=['query1','query2','label'])
    lc_df = pd.read_csv('lc_df.csv',names=['query1','query2','label'])
    oppo_df = pd.read_csv('oppo_df.csv',names=['query1','query2','label'])


    oppo_df.dropna(inplace=True)
    print(oppo_df.isnull().any())
    big_df=[lc_df,bq_df,oppo_df]#拼成大的list
    indexes=[]
    ...
    for df in big_df:
        split_index=[(train_index , dev_index) for train_index , dev_index in skf.split(df.index,df['label'])]#分割lc_df,bq_df,oppo_df三个训练集,split_index里面有五个
        indexes.append(split_index)#加入序号集,有三个
    
    split_datas=[]
    for i in range(5):
        train_df=[]
        dev_df=[]
        for ind,df in zip(indexes,big_df):
            train_index,dev_index=ind[i]
            train_df.append(df.iloc[train_index])#加入各自的训练集
        ...
        print('{}_fold的train的大小:{}'.format(i+1,train_df.shape))
        print('{}_fold的dev的大小:{}'.format(i+1,dev_df.shape))
    #返回五折的数据
    return split_datas

def train_model(model,train_data_loader,dev_data_loader,model_save_path,num_training_steps,rank,tokenizer):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            ce_loss = criterion(logits1, labels)#交叉熵损失
            ...
            if global_step % args.eval_step == 0 and rank == 0:#每十步打印验证信息
                #验证集的acc
                accuracy = evaluate(model, criterion, metric, dev_data_loader)
                
                #保存最好的模型
                if accuracy > best_accuracy:
                    save_dir = os.path.join('./model_data/'+model_save_path, "model_%d" % global_step)
                    print(save_dir)
                    
                    if not os.path.exists(save_dir):
                        os.makedirs(save_dir)#创建保存路径
...
def do_train_5fold():
    ...
    split_datas=stratifiedk_5fold()

    for df in split_datas:
        ...
        #将train_ds转为MapDataset
        tra=df[0].to_dict(orient='records')
        val=df[1].to_dict(orient='records')
        ...
        #构建模型
        model = QuestionMatching_base(pretrained_model, rdrop_coef=args.rdrop_coef)
        ...
        print('{}fold==========>num_training_steps:{}'.format(i+1,num_training_steps))
        
        model_save_path=args.model_name+'/'+args.save_checkpoints_name+'/'+str(i+1)+'fold'
        #训练
        train_model(model,train_data_loader,dev_data_loader,model_save_path=model_save_path,num_training_steps=num_training_steps,rank=rank,tokenizer=tokenizer)
